...
import { generateSyntheticProfile } from '../utils/shotUtils';
...
export const useDataManagement = (shots: ShotData[]) => {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        try {
          await navigator.share({
            ...
            text: 'Alege unde doresti sa salvezi fisierul Excel.'
          });
        } catch (e) {
  ...
  const handleBackupCloud = async () => {
    try {
      ...
      const shareName = originalName + ".txt";
      const file = new File([dataStr], shareName, { type: 'text/plain' });

      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        try {
          await navigator.share({
            files: [file],
            title: 'Backup Pharmabarista',
            text: 'Salveaza fisierul de backup (Format compatibil .txt)'
          });
        } catch (shareError: any) {
          if (shareError.name !== 'AbortError') {
            console.error("Share failed, falling back:", shareError);
            alert("Partajarea a esuat. Se descarca local.");
            const blob = new Blob([dataStr], { type: 'application/json' });
            forceDownload(blob, originalName);
          }
        }
      } else {
      ...
      console.error("Cloud Backup generation error:", error);
      alert("A aparut o eroare la generarea backup-ului.");
    }
  };
    ...
    reader.onload = async (ev) => {
      try {
        ...
        if (!imported || typeof imported !== 'object') {
          alert("Structura fisier invalida.");
          return;
        }
        if (!imported.meta || imported.meta.app !== "Pharmabarista AI") {
          alert("Fisierul nu provine din aplicatia PharmaBarista AI (Semnatura lipsa).");
          return;
        }
        if (imported.shots && !Array.isArray(imported.shots)) throw new Error("Format invalid: shots trebuie sa fie array.");
        if (imported.machines && !Array.isArray(imported.machines)) throw new Error("Format invalid: machines trebuie sa fie array.");
        if (imported.beans && !Array.isArray(imported.beans)) throw new Error("Format invalid: beans trebuie sa fie array.");

        if (confirm(`Backup valid din data ${new Date(imported.meta.date).toLocaleDateString()}.\n\nSigur doresti sa importi datele? Aceasta actiune va actualiza baza de date curenta.`)) {
          try {
            ...
            alert("Eroare la scrierea datelor in baza de date. Verificati integritatea fisierului.");
          }
        }
      } catch (err) {
        console.error(err);
        alert(`Eroare critica la import: ${err}`);
      } finally {
