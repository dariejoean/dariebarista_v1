...
// --- EMERGENCY RECOVERY ---
// STABILITY FIX: Use Dexie.delete() with DB name instead of unsafe (db as any).delete()
...
// --- SHOT METHODS ---
// STABILITY FIX: Wrapped in a Dexie transaction for full atomicity
          ...
          await db.transaction('rw', db.shots, async () => {
                  for (const shot of shots) {
                            if (shot.grindScaleType === 'eureka' && shot.grindSetting && shot.grindSetting >= 20) {
                                        ...
                                        await db.shots.update(shot.id, { grindSetting: newGrindSetting });
                            }
                  }
          });
...
// STABILITY FIX: Wrapped in Dexie transaction - both tables clear atomically or neither does
export const clearAllProducts = async (): Promise<void> => {
    try {
          await db.transaction('rw', db.machines, db.beans, async () => {
